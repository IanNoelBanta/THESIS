...
while True:
    ...
    results = model.track(frame, verbose=False, classes=[1], stream_buffer=True, conf=0.8, persist=True, tracker="botsort.yaml")
    ...
    if results[0]:
        cv2.putText(frame, f"DETECTED:{len(results[0])} TRACKING: {results[0][0].boxes.id}", (0, 50), FONT, FONTSCL, COLOR, THICKNESS, cv2.LINE_AA)
        frame = results[0].plot()
        if len(results[0]) >= 1:
            results = results[0][0]
            targetX1 = int(results.boxes.xyxy.cpu().numpy()[0][0])
            targetY1 = int(results.boxes.xyxy.cpu().numpy()[0][1])
            targetX2 = int(results.boxes.xyxy.cpu().numpy()[0][2])
            targetY2 = int(results.boxes.xyxy.cpu().numpy()[0][3])
